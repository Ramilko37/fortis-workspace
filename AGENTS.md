...
Пользователь заранее разрешает агентам работать с Linear в рамках Fortis без дополнительных подтверждений:

- читать, создавать и обновлять задачи, статусы, labels, relations и комментарии;
- указывать в Linear внутренние пути файлов репозитория, номера строк, результаты `lint`, `build`, тестов, React Doctor и других dev-инструментов;
